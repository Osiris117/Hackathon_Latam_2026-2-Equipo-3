...
def genetico():
    ...
    # Reconstrucción del almacenamiento utilizando la mejor solución
    S_actual = np.zeros(T)
    S = S_inicial

    for i in range(T):
        S_actual[i] = S
        S = S + Delta_S_obs[i] - mejor_secuencia[i]
    
    df_hist = pd.DataFrame({
        "R_obs": R_obs,
        "Delta_S_obs": Delta_S_obs,
        "S_actual": S_actual
    })
    ...
    return {
        ...
        "df_hist": df_hist,
        "mejor_secuencia": np.array(mejor_secuencia),        
        "R_obs": R_obs,
        "Delta_S_obs": Delta_S_obs,
        "S_inicial": S_inicial,
        "S_max": S_max,
